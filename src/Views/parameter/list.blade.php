...
@section('params-content')
    <div class="btn-toolbar mb-1" role="toolbar" aria-label="Toolbar with button groups">
        ...
        <div class="btn-group form-switch-group" role="group" aria-label="">
            <a class="btn btn-default{{ request()->get('show') === 'all' ? ' disabled' : '' }}" href="{{ url('params/parameter?show=all') }}">@lang('params::all.all')</a>
            <a class="btn btn-default{{ (!request()->has('show') || request()->get('show') === 'enabled') ? ' disabled' : '' }}" href="{{ url('params/parameter?show=enabled') }}">@lang('params::all.enabled')</a>
            <a class="btn btn-default{{ request()->get('show') === 'disabled' ? ' disabled' : '' }}" href="{{ url('params/parameter?show=disabled') }}">@lang('params::all.disabled')</a>
        </div>
    </div>
    @foreach ($parameters as $parameter)
    <div class="card mb-2 mt-2">
        <div class="card-header">
            <h3><a href="{{ url('params/parameter/' . $parameter->id) }}">#{{ $parameter->id }} {{ $parameter->name }}</a></h3>
