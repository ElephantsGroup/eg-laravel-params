...
@section('params-content')
    <div class="btn-toolbar mb-1" role="toolbar" aria-label="Toolbar with button groups">
        ...
        <div class="btn-group form-switch-group" role="group" aria-label="">
            <a class="btn btn-default{{ request()->get('show') === 'all' ? ' disabled' : '' }}" href="{{ url('params/template?show=all') }}">@lang('params::all.all')</a>
            <a class="btn btn-default{{ (!request()->has('show') || request()->get('show') === 'enabled') ? ' disabled' : '' }}" href="{{ url('params/template?show=enabled') }}">@lang('params::all.enabled')</a>
            <a class="btn btn-default{{ request()->get('show') === 'disabled' ? ' disabled' : '' }}" href="{{ url('params/template?show=disabled') }}">@lang('params::all.disabled')</a>
        </div>
    </div>
    @foreach ($templates as $template)
    <div class="card mb-2 mt-2{{ ($activeTemplate && $activeTemplate->template_id === $template->id) ? ' selected-card' : ''}}">
        <div class="card-header">
            <h3><a href="{{ url('params/template/' . $template->id) }}">#{{ $template->id }} {{ $template->name }}</a></h3>
            @if (!$activeTemplate || $activeTemplate->template_id != $template->id)
            <form class="btn-group mr-2" role="group" aria-label="" action="{{ url('params/active-template') }}" method="POST">
                @csrf
